...
def get_night_traffic_by_tile_service_time_city(traffic_type: TrafficType, start_night: time, end_night: time, city: List[City] = None, service: List[Service] = None, remove_noisy_nights: bool = True):
...
def get_night_traffic_by_tile_service_city(traffic_type: TrafficType, start_night: time, end_night: time, city: List[City] = None, service: List[Service] = None, remove_noisy_nights: bool = True):
    ...
    traffic_data = traffic_data.groupby(group='time').sum()
    return traffic_data


def get_night_traffic_city_by_tile_service_time(city: City, traffic_type: TrafficType, start_night: time, end_night: time, service: List[Service], remove_noisy_nights: bool = True, expand_city_dimension: bool = False):
    traffic_data_city = []
    ...
    if expand_city_dimension:
        traffic_data_city = traffic_data_city.expand_dims(dim={TrafficDataDimensions.CITY.value: [city.value]})
